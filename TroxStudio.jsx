...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      let p = null;
      try { const r = await window.storage.get("trox_profile"); if (r?.value) p = JSON.parse(r.value); } catch (e) {}
      if (!p) { p = TROX_DEFAULT; try { await window.storage.set("trox_profile", JSON.stringify(p)); } catch (e) {} }
      setProfile(p);
      try { const r = await window.storage.get("trox_playbook"); if (r?.value) setPlaybook(r.value); } catch (e) {}
      try { const r = await window.storage.get("trox_posts"); if (r?.value) setPosts(JSON.parse(r.value)); } catch (e) {}
      try { const r = await window.storage.get("trox_followers"); if (r?.value) setFollowers(JSON.parse(r.value)); } catch (e) {}
      setLoaded(true);
    })();
  }, []);

  const persist = async (k, v) => { try { await window.storage.set(k, typeof v === "string" ? v : JSON.stringify(v)); } catch (e) {} };
  const savePosts = (n) => { setPosts(n); persist("trox_posts", n); };
  const saveFollowers = (n) => { setFollowers(n); persist("trox_followers", n); };
  async function saveProfile() { setProfile({ ...draft }); setEditing(false); await persist("trox_profile", draft); }
